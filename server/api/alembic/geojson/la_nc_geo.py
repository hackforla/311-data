...
with open(DATA_FOLDER + "Neighborhood Councils (Certified).geojson", "r") as read_file:
...
with open(DATA_FOLDER + "nc_data.csv", "w") as write_file:
    council_df.drop(columns=['geometry']).to_csv(write_file)

# export a CSV with the centroids
with open(DATA_FOLDER + "nc_data.json", "w") as write_file:
    council_df[['council_name', 'centroid']].to_json(write_file, orient='records', lines=True, default_handler=str)
...
with open(DATA_FOLDER + "regions_data.csv", "w") as write_file:
